...
export default function DocumentDropZone(props: Props) {
    ...
    return (
        ...
        <div
            ...
            <input {...getInputProps()} />
            <p>Clique para selecionar ou arraste o arquivo dentro desta área.</p>
            <em>(Somente os formatos *.doc, *.docx e *.pdf serão aceitos)</em>
        </div>
        <aside>
            {files.map((file: any) => (
                <li key={file.path} className={classes.item}>
                    <FlexBox gap={styles.size(1)}>
                        <Icon className={classes.icon} name={icons.docLine} />
