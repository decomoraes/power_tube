...
type Props = {
    closeModal: () => void;
    onPictureSubmit: (file: any) => void;
}

// region UploadFileModal
export default function UploadDocumentFileModal(props: Props) {
    // region properties
    const theme: any = useTheme();
    const classes = useStyles({theme});
    const wrapperRef = useRef(null);
    const [files, setFiles] = useState<any>([])
    useOutsideEvent(wrapperRef, () => props.closeModal());
    // endregion
        ...
        >
            <FlexBox
                ...
                <FlexBox
                    ...
                    <h2 className={classes.title}>Escolha o arquivo</h2>
                    <div
                ...
                <DocumentDropZone onChange={(e) => setFiles(e)} />
                <Button
                    // disabled={props.files.length === 0}
                    onClick={() => props.onPictureSubmit(files[0])}>
                    Abrir
                </Button>
            </FlexBox>
        </div>
